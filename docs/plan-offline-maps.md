...
- **iOS:** Safari מוחק את האחסון של אתר אחרי 7 ימים בלי אינטראקציה; אפליקציות
  שנוספו למסך הבית פטורות ([WebKit](https://webkit.org/blog/14403/updates-to-storage-policy/),
  [MDN](https://developer.mozilla.org/en-US/docs/Web/API/Storage_API/Storage_quotas_and_eviction_criteria)).
  לכן ההודעה באפליקציה: "הוסף את Navi למסך הבית ופתח משם".
- **Android/Chrome:** `navigator.storage.persist()` מתבקש בהורדה הראשונה.
...
5. לחזור: אותו מתג.
6. **אייפון אמיתי:** שיתוף → הוסף למסך הבית → לפתוח משם → להוריד מסלול → מצב טיסה
   → לפתוח שוב. ה-GPS זז על המפה.
...
| Safari מוחק אחסון (7 ימים) | הודעה להוסיף למסך הבית |
...
| RTL plugin ב-worker לא נתפס ע"י ה-SW בספארי | תוויות עברית ייראו הפוכות. בכרום עובד; באייפון — לבדוק |
...
- לבדוק על אייפון ממסך הבית (RTL, GPS, wake lock).
- ייצור הקריינות נכשל בבדיקה מסיבה לא קשורה: Gemini הודיע ש-`gemini-2.5-flash`
  לא זמין יותר למשתמשים חדשים (`narration.ts:103`, `tts.ts:78`). לעדכן דגמים.
